...
for i_episode in range(10000000):
    ...
    while True:
        ...
        with g2.as_default():
            if done_2 == 0:
                r_action_number_2 = RL_2.choose_action(state_2)
            else:
                r_action_number_2 = 9
        with g3.as_default():
            if done_3 == 0:
        ...
        '''绘图'''
        actionlist1.append(r_action_number)
        actionlist2.append(r_action_number_2)
        ...
        with g2.as_default():
            RL_2.store_transition(situation_information_2, r_action_number_2, reward_global/4, situation_information_next_2)
        with g3.as_default():
            RL_3.store_transition(situation_information_3, r_action_number_3, reward_global/4, situation_information_next_3)
        with g4.as_default():
            RL_4.store_transition(situation_information_4, r_action_number_4, reward_global/4, situation_information_next_4)
        ...
        if done_all:
            if i_episode % 1000 ==0 and i_episode!=0:
                ...
                with g1.as_default():
                    RL.storevariable()
                with g2.as_default():
                    RL_2.storevariable_2()
                with g3.as_default():
                    RL_3.storevariable_3()
                with g4.as_default():
                    RL_4.storevariable_4()
                print('**************** check point *****************')
                print(b_position)
                print(b_position_2)
                print(b_position_3)
                ...
                plt.savefig("{}{}{}{}{}{}{}{}{}.png".format(i_episode,title,ep_r,'_',ep_r_2,'_',ep_r_3,'_',ep_r_4), dpi=300)
                ...
                for i in range(len(X)):
                    csv_writer.writerow(
                        [i, "1", X[i] * 100, Y[i] * 100, Z[i] * 100, "1", X_B[i] * 100, Y_B[i] * 100, Z_B[i] * 100])
                for i in range(len(X_2)):
                    csv_writer.writerow(
                        [i, "2", X_2[i] * 100, Y_2[i] * 100, Z_2[i] * 100, "2", X_B_2[i] * 100, Y_B_2[i] * 100,
                         Z_B_2[i] * 100])
                for i in range(len(X_3)):
                    csv_writer.writerow(
                        [i, "3", X_3[i] * 100, Y_3[i] * 100, Z_3[i] * 100, "3", X_B_3[i] * 100, Y_B_3[i] * 100,
                         Z_B_3[i] * 100])
                for i in range(len(X_4)):
                    csv_writer.writerow(
                        [i, "4", X_4[i] * 100, Y_4[i] * 100, Z_4[i] * 100, "4", X_B_4[i] * 100, Y_B_4[i] * 100,
                         Z_B_4[i] * 100])
                f.close()
            break
        total_steps += 1
